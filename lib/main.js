...
const run = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const publish = () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        */
        ...
        try {
            ({
                data: { upload_url }
            } = yield octokit.repos.getRelease(Object.assign(Object.assign({}, repo), { release_id: releaseId })));
        }
        catch (error) {
            // return setFailed(error.message);
            console.log(error);
        }
        const filepath = glob_1.default.sync("./report/**/*.*", {
            absolute: true,
            nocase: true
        });
    ...
    let targetHash = "";
    try {
        targetHash = child_process_1.execSync(`git merge-base -a origin/${event.pull_request.base.ref} origin/${event.pull_request.head.ref}`, { encoding: "utf8" }).slice(0, 7);
    }
    catch (e) {
        targetHash = child_process_1.execSync(`git merge-base -a origin/${event.pull_request.base.ref} pull/${event.number}/merge`, { encoding: "utf8" }).slice(0, 7);
    }
    if (!targetHash) {
        console.error("fail");
        return;
    ...
    const contents = yield octokit.repos
        .getContents(Object.assign(Object.assign({}, repo), { path: `${targetHash}/actual`, ref: BRANCH_NAME }))
        .catch(() => {
        return { data: [] };
    });
