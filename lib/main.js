...
console.log(event);
const targetHash = child_process_1.execSync(`git merge-base -a origin/${event.pull_request.base.ref} origin/${event.pull_request.head.ref}`, { encoding: "utf8" }).slice(0, 8);
...
const run = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    yield Promise.all(glob_1.default.sync("./report/**/*.*").map((p) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        tree.data.tree.push({
            path: path
                .join(`reg-${event.after.slice(0, 8)}-${timestamp}`, p)
                .replace(/^\.\//, ""),
            mode: "100644",
            type: "blob",
            sha: blob.data.sha
